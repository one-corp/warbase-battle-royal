...
### 4.2 Networked Hitscan Implementation

```typescript
function fireHitscan(scene: Scene, camera: Camera, config: WeaponConfig, networkManager: NetworkManager) {
    // 1. Calculate spread based on movement/recoil bloom
    const spreadAngle = calculateSpread(config);
    const spreadRotation = Math.random() * Math.PI * 2;
    
    // 2. Compute firing vector
    ...
    // 3. Cast Ray from Camera
    const ray = new Ray(camera.position, spreadDir, config.range || 200);
    const hit = scene.pickWithRay(ray);
    
    if (hit?.hit && hit.pickedMesh) {
        
        // 4. Resolve Environment Physics
        if (hit.pickedMesh.physicsBody) {
            hit.pickedMesh.physicsBody.applyImpulse(spreadDir.scale(10), hit.pickedPoint!);
        }

        // 5. Resolve Player Hits via Metadata Tagging
        if (hit.pickedMesh.metadata && hit.pickedMesh.metadata.playerId) {
            const targetId = hit.pickedMesh.metadata.playerId;
            
            // Transmit damage to authoritative server
            networkManager.sendHit(targetId, config.damage);
            
            // Trigger local hitmarker UI
            showHitmarkerUI();
        }
    }
}
...
*Note: For the complete network architecture regarding damage resolution, death states, and scoreboard tracking, see `07_HIT_PHYSICS.md`.*
